...
// PLEASE ENSURE x,y,z are in the range -1 to 1
typedef struct {
  float x;
  float y;
  float z;
...
const float ERR_THRESH = 0.5; // OMG HARRISON LOOK PLEASE CONFIGURE THIS AFTER TESTING AT A LATER DATE BECAUSE THIS IS A GUESS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
const int ROT_TIME_MILLIS = 20; // AHHHHHHHHH HARRISON ADJUST THIS HIGHER WILL HAVE A MORE PROFOUND EFFECT OF ROTATION, LOWER IS LESS DELAY IN DETECTING CHANGES OF ANGLES!!!!!!!!!!!!!!!!!!!! 
...
void setup() {
  ...
  for (int cal_int = 0; cal_int < 2000 ; cal_int ++){                  //Run this code 2000 times
    if(cal_int % 125 == 0){
       Serial.println(".");                              //Print a dot on the LCD every 125 readings
    }
...
void loop(){
  ...
  // RUN ONE ITERATION OF ROTATE FUNCTION TO MOVE TOWARD GOAL
  vec dest; //TODO FILL IN WITH NAVIGATION LOGIC
  move_towards(dest);
...
void write_LCD(){                                                      //Subroutine for writing the LCD
  ...
  if(lcd_loop_counter >= 1 && lcd_loop_counter <= 7) {
...
void setup_mpu_6050_registers(){
  ...
  Wire.beginTransmission(0x68);                                        //Start communicating with the MPU-6050
  Wire.write(0x1B);                                                    //Send the requested starting regv- ister
  Wire.write(0x08);                                                    //Set the requested starting register
  Wire.endTransmission();                                              //End the transmission
}

// HARRISON MAKE SURE THAT ch1 and ch2 ARE [-1,1]
void rot(float ch1, float ch2) {
  float tail_left, tail_right;
  tail_left = ch1 - ch2;
  tail_right = ch1 + ch2;

  // Find the larger of the two numbers' magnitude to act as our normalizer
  float normalizer = max(abs(tail_left),abs(tail_right));
  if(normalizer > 1) {
    // Not zero, so we can divide
    // Correct to within the range [-1, 1] while keeping ratios constant
    tail_left /= normalizer;
    tail_right /= normalizer;
  }
  // Map from [-1,1] to [0, 180]
  tail_left = (tail_left + 1)*90;
  tail_right = (tail_right + 1)*90;

  // TODO map to servo range and plug into servo so it rotates to the given point
}

float hypot(vec a, vec b) {
  return sqrt(pow(b.x-a.x,2) + pow(b.y-a.y,2) + pow(b .z-a.z,2));
}

// Synchronous non waiting function to move the plane towards its desitnation
float move_towards(vec dest) {
  // Find difference between where we are and where we need to be
  vec curr = {curx,cury,0}; // Initializes the cur vector
  if(hypot(dest,curr) > ERR_THRESH) {
    rot(dest.x-curr.x,dest.y-curr.y);
    
    // TODO UPDATE CURX and CURY
    curr.x = curx;
    curr.y = cury;
    return hypot(dest,curr);
  } else {
    return 0;
  } 
}
...
void pid(){
  if(mem.x != targ_vec.x && mem.y != targ_vec.y && mem.z != targ_vec.z)
  {
    if(keepout == 0)
    {
      targx=(atan(targ_vec.x/targ_vec.z)*(57.29577951308)+curx);
      targy=(atan(targ_vec.y/targ_vec.z)*(57.29577951308)+cury);
      keepout=1;
    }
    if(curx + 1 <= targx && curx - 1 >= targx && cury + 1 <= targy && cury - 1 >= targy) {
      // TODO FILL IN
      int walalalalalalalalal = 1;
    }
  }
  else
  {
